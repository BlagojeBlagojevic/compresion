	...
	#pragma omp parallel
	{
		#pragma omp single
		{
			for(size_t i = 0; i < (size_t)NUM_OF_THREADS; i++){
				#pragma omp task
				{
					size1[i] = fastlz_decompress(compressed[i].array,compressed[i].index_of_last
																			 ,decompressed[i].array,MAX_FILE_SIZE);
					printf("\nsize %d\n\n", size1[i]);
				}	
			}
...
int main(int argc, char *argv[]) {
	...
	compression("etf.txt", "2");
	deccompression("etfd.txt","2");
	//system("pause");
	}
